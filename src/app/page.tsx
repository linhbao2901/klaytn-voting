...
import VotingContract from "../contracts/DistributedVoting.json"; // Replace with the path to your compiled contract JSON
...
export default function Home() {
  ...
  useEffect(() => {
    const init = async () => {
      if (window.ethereum) {
        ...
        const contractAddress = "0x62f7B2Ce5488f7c2F4Cc0B4201EBf35F65c06cF1"; // Replace with the deployed contract address
  ...
  return (
    <div className={styles.container}>
      <h1 className={styles.heading}>Voting DApp on Klaytn chain</h1>
      {!provider ? (
        ...
        <div>
          <h2 className={styles.subheading}>Please select your option to vote:</h2>
          {renderOptions()}
          {isContractOwner && (
                <div>
                  <input
                  ...
                  <button className={styles.button} onClick={() => handleAddOption(votingContract)}>
                    Add Vote Option (Only Admin)
                  </button>
                </div>
              )}
          {hasVoted ? (
            <p className={styles.message}>You have already submited your vote!</p>
          ) : (
